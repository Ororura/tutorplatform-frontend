    ...
    <section aria-label="Сводка преподавателя" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {cards.map((card) => {
        ...
        return (
          <Link
            key={card.label}
            className="group rounded-[14px] border border-[var(--border)] bg-white p-4 transition hover:border-blue-200"
            href={card.href}
          >
            <div className="flex items-start justify-between gap-3">
              <span className={`flex size-8 items-center justify-center rounded-lg ${card.iconClassName}`}>
                <Icon size={17} />
              </span>
