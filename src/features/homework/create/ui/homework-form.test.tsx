...
const mocks = vi.hoisted(() => ({ useQuery: vi.fn(), create: vi.fn(), update: vi.fn(), push: vi.fn(), back: vi.fn(), pending: false }));
...
vi.mock("../api/homework-mutations", () => ({ useCreateHomeworkMutation: () => ({ mutateAsync: mocks.create, isPending: mocks.pending }), useUpdateHomeworkMutation: () => ({ mutateAsync: mocks.update, isPending: false }) }));
import { localDateTimeToIso } from "@/entities/session";
import { HomeworkForm } from "./homework-form";

const program = (id: string, title: string) => ({ id, learningProgramId: `learning-${id}`, title, status: "ACTIVE", reportIntervalMinutes: 480, startedAt: "2026-09-01T00:00:00Z", subject: { id: `subject-${id}`, name: `Предмет ${id}` } });
const task = (id: string, title: string, taskType: "TEXT" | "CODE") => ({ id, subjectId: "subject-p1", title, descriptionMarkdown: "Описание", taskType, difficulty: id === "text" ? "EASY" : "MEDIUM", status: "ACTIVE", version: 0, createdAt: "2026-01-01T00:00:00Z", updatedAt: "2026-01-01T00:00:00Z" });
const queryResult = (data: unknown) => ({ data, isPending: false, isError: false, isFetching: false, refetch: vi.fn() });
const taskPage = { items: [task("text", "Текст", "TEXT"), task("code", "Код", "CODE")], page: 0, size: 10, totalElements: 2, totalPages: 1 };

describe("HomeworkForm", () => {
  let programs = [program("p1", "Python")];
  beforeEach(() => {
    programs = [program("p1", "Python")]; mocks.create.mockReset().mockResolvedValue({ id: "homework-new" }); mocks.update.mockReset(); mocks.push.mockReset(); mocks.pending = false;
    mocks.useQuery.mockImplementation((options?: { queryKey?: unknown[]; enabled?: boolean }) => options?.queryKey?.includes("student-programs") ? queryResult(programs) : queryResult(taskPage));
  });
  ...
  it("sends atomic ordered items, required flags, title, description and ISO dueAt", async () => {
    ...
    await waitFor(() => expect(mocks.create).toHaveBeenCalledWith({
      studentProgramId: "p1", title: "Практика", description: "Два задания", dueAt: localDateTimeToIso("2026-09-20T18:30"),
      items: [{ taskId: "code", required: false, position: 0 }, { taskId: "text", required: true, position: 1 }],
    }));
