...
const program = (id: string, title: string) => ({
  ...
  subject: { id: "subject-1", name: "Информатика" },
});
const details = (id: string, moduleTitle = "Основы Python", topicTitle = "Переменные") => ({
  ...program(id, id === "p1" ? "Python" : "Алгоритмы"),
  modules: [
    {
      id: `module-${id}`,
      title: moduleTitle,
      position: 0,
      topics: [
        { id: `topic-${id}`, title: topicTitle, position: 0, topicStatus: "ACTIVE", progressStatus: "AVAILABLE" },
      ],
    },
  ],
});
...
const queryResult = (data: unknown, overrides: Record<string, unknown> = {}) => ({
  data,
  isPending: false,
  isError: false,
  refetch: vi.fn(),
  ...
  beforeEach(() => {
    ...
    mocks.useQuery.mockImplementation((options: { queryKey: unknown[]; enabled?: boolean }) => {
      ...
      return options.enabled === false
        ? queryResult(undefined, { isPending: false })
        : queryResult(programDetails[id as keyof typeof programDetails] ?? programDetails.p1);
    });
  });
  ...
  it("shows optimistic-lock conflict guidance", async () => {
    mocks.update.mockRejectedValue(
      new ApiClientError(409, {
        code: "LESSON_SESSION_VERSION_CONFLICT",
        message: "internal",
