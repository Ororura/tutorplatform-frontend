...
vi.mock("next/navigation", () => ({
  ...
  usePathname: () => "/teacher/students/alex/homework",
...
describe("TeacherStudentHomeworksView", () => {
  ...
  it("shows the empty state and blocks creation without programs", () => {
    render(<TeacherStudentHomeworksView studentId="alex" />);
    expect(screen.getByText("Домашних заданий пока нет")).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /Программа/ })).toHaveAttribute("href", "/teacher/students/alex/program");
    expect(screen.queryByRole("link", { name: "Назначить домашнее задание" })).not.toBeInTheDocument();
  });
  it("shows create action with a program and sends status through URL", () => {
    mocks.useQuery.mockImplementation((options?: { queryKey?: unknown[] }) =>
      options?.queryKey?.includes("student-programs")
        ? {
            data: [{ id: "p1", title: "Python" }],
            isPending: false,
            isError: false,
          }
        : { data: emptyPage, isPending: false, isError: false, isFetching: false, refetch: vi.fn() },
