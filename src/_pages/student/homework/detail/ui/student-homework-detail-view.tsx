...
export function StudentHomeworkDetailView({ homeworkId }: Readonly<{ homeworkId: string }>) {
  ...
  if (homework.isError) {
    ...
    return (
      <main className="mx-auto max-w-4xl space-y-4 px-6 py-12">
        ...
        <Link className="underline" href="/student/homework">
  ...
  return (
    <main className="mx-auto max-w-4xl space-y-8 px-6 py-12">
      <header>
        <Link className="text-sm text-neutral-600 underline" href="/student/homework">
          ← Домашние задания
        </Link>
      ...
      <section className="space-y-5 rounded-lg border bg-white p-6">
        <dl className="grid gap-4 sm:grid-cols-3">
          <div>
            ...
            <dd>{formatHomeworkDate(data.assignedAt)}</dd>
          </div>
          <div>
            <dt className="text-sm text-neutral-500">Срок</dt>
            <dd>{data.dueAt ? formatHomeworkDate(data.dueAt) : "Без срока"}</dd>
          </div>
          <div>
            <dt className="text-sm text-neutral-500">Выполнено</dt>
            <dd>{data.completedAt ? formatHomeworkDate(data.completedAt) : "—"}</dd>
          </div>
        </dl>
          ...
          {items.map((item) => {
            ...
            return (
              <li className="flex flex-wrap items-center gap-3 p-4" key={item.id}>
                ...
                {supported ? (
                  <Button type="button" onClick={() => setOpenedItemId(item.id)}>
