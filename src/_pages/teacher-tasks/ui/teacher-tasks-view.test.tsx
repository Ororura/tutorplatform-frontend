...
describe("TeacherTasksView", () => {
  beforeEach(() => {
    ...
    mocks.useQuery.mockImplementation((options?: { queryKey?: unknown[] }) =>
      options?.queryKey?.includes("subjects")
        ...
        : { data: page, isPending: false, isError: false, isFetching: false, refetch: vi.fn() },
    );
  });
  ...
  it("writes supported filters to URL so the query receives them", () => {
    render(<TeacherTasksView />);
    fireEvent.change(screen.getByLabelText("Статус"), { target: { value: "ACTIVE" } });
    expect(mocks.replace).toHaveBeenCalledWith("/teacher/tasks?status=ACTIVE", { scroll: false });
  });
  it("shows loading and retryable errors", () => {
    const refetch = vi.fn();
    mocks.useQuery.mockReturnValue({ isPending: true, isError: false, refetch });
    const { rerender } = render(<TeacherTasksView />);
    expect(screen.getByText("Загружаем задания…")).toBeInTheDocument();
    mocks.useQuery.mockReturnValue({ isPending: false, isError: true, refetch });
    rerender(<TeacherTasksView />);
