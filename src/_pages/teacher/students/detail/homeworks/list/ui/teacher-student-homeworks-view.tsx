...
export function TeacherStudentHomeworksView({ studentId }: Readonly<{ studentId: string }>) {
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-8 px-6 py-12">
      <div>
        <Link className="text-sm text-neutral-600 underline" href={`/teacher/students/${studentId}`}>
          ← Профиль ученика
        </Link>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <select
          ...
          className="h-10 rounded-md border border-neutral-300 px-3"
          value={status ?? ""}
          onChange={(event) => navigate({ status: event.target.value || undefined, page: undefined })}
        >
      ...
      {homeworks.data?.items.length === 0 && (
        <div className="space-y-4 rounded-lg border border-dashed p-8 text-center">
          ...
          {programs.data && programs.data.length > 0 ? (
            ...
            <>
              <p className="text-sm text-neutral-600">Сначала назначьте ученику программу обучения</p>
              <Link className="underline" href={`/teacher/students/${studentId}/program`}>
      ...
      {homeworks.data && homeworks.data.totalPages > 1 && (
        <nav className="flex items-center justify-between" aria-label="Пагинация домашних заданий">
          <button
