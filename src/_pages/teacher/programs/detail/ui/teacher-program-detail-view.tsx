...
export function TeacherProgramDetailView({ programId }: Readonly<{ programId: string }>) {
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6">
      <Link className="inline-flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-blue-600" href="/teacher/programs">
        ← Программы обучения
      </Link>

      {program.isPending && (
        <div className="rounded-[28px] border border-white/80 bg-white p-6 text-sm text-slate-500 shadow-[0_12px_40px_rgba(45,79,135,0.06)]" aria-busy="true">
          Загружаем программу…
        </div>
      )}
      ...
      {program.isError && (
        <section className="space-y-3 rounded-[28px] border border-red-100 bg-red-50 p-6" role="alert">
          <h1 className="text-xl font-semibold text-slate-950">{notFound ? "Программа не найдена" : "Не удалось загрузить программу."}</h1>
          <p className="text-sm text-slate-600">
            {notFound ? "Возможно, программа была удалена или у вас нет к ней доступа." : "Попробуйте обновить страницу ещё раз."}
          </p>
          {!notFound && (
          ...
          <>
            <section className="rounded-[28px] border border-white/80 bg-white p-6 shadow-[0_12px_40px_rgba(45,79,135,0.06)] sm:p-7">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${statusClassName[program.data.status]}`}>
                  <StatusIcon size={13} />
              ...
              <section className="mt-6" aria-labelledby="program-description-heading">
                <h2 id="program-description-heading" className="text-sm font-semibold text-slate-950">Описание</h2>
                <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-500">{program.data.description || "Описание программы пока не добавлено."}</p>
              </section>
            </section>

            <section className="rounded-[28px] border border-white/80 bg-white p-6 shadow-[0_12px_40px_rgba(45,79,135,0.06)] sm:p-7" aria-labelledby="program-modules-heading">
              <div className="flex items-center gap-3">
                <span className="flex size-10 items-center justify-center rounded-xl bg-blue-50 text-blue-600"><Layers3 size={19} /></span>
                <div>
                  <h2 id="program-modules-heading" className="text-xl font-semibold text-slate-950">Модули программы</h2>
                  <p className="mt-1 text-sm text-slate-500">{modules.length} {modules.length === 1 ? "модуль" : "модулей"}</p>
                </div>
              </div>
                  ...
                  {modules.map((module) => {
                    ...
                    return (
                      <li key={module.id} className="rounded-2xl border border-slate-200/80 p-5">
                        <p className="text-xs font-medium uppercase tracking-wide text-blue-600">Модуль {module.position + 1}</p>
                            ...
                            <ol className="mt-3 space-y-2">
                              {topics.map((topic) => (
                                <li key={topic.id} className="flex items-start gap-2 text-sm text-slate-700">
                                  ...
                                  <div><span className="font-medium">{topic.title}</span>{topic.description && <p className="mt-0.5 text-slate-500">{topic.description}</p>}</div>
                                </li>
                              ))}
                            </ol>
                          )}
