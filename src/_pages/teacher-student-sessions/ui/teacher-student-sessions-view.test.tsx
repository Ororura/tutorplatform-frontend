...
vi.mock("next/navigation", () => ({
  ...
  usePathname: () => "/teacher/students/student-1/sessions",
...
describe("TeacherStudentSessionsView", () => {
  ...
  function setQueries(sessionResult: ReturnType<typeof result>, programData: unknown = programs) {
    mocks.useQuery.mockImplementation((options: { queryKey: unknown[] }) =>
      options.queryKey.includes("lesson-sessions") ? sessionResult : result({ data: programData }),
    );
  }
  ...
  it("shows empty state and create action when a program exists", () => {
    ...
    expect(screen.getAllByRole("link", { name: "Добавить занятие" })[0]).toHaveAttribute(
      "href",
      "/teacher/students/student-1/sessions/new",
    );
  });
  ...
  it("blocks create and links to Program when there are zero programs", () => {
    ...
    expect(screen.getByRole("link", { name: /Перейти в раздел/ })).toHaveAttribute(
      "href",
  ...
  it("uses server pagination and stores page in URL", () => {
    ...
    const sessionOptions = mocks.useQuery.mock.calls.find(([options]) =>
