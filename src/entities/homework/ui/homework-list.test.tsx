...
describe("HomeworkList", () => {
  it("renders persisted states plus backend-derived overdue and detail links", () => {
    ...
    ["Назначено", "Просрочено", "Выполнено", "Отменено"].forEach((label) => expect(screen.getByText(label)).toBeInTheDocument());
    expect(screen.getByRole("link", { name: /Прошлое/ })).toHaveAttribute("href", "/teacher/students/alex/homework/overdue");
