...
describe("sessionQueries", () => {
  ...
  it("includes studentId and server pagination/sort in request and query key", async () => {
    ...
    expect(getMock).toHaveBeenCalledWith("/api/v1/teacher/students/{studentId}/sessions", { params: { path: { studentId: "student-alex" }, query: { page: 2, sort: "startedAt,desc" } } });
    expect(sessionQueries.list("student-alex", { page: 0 }).queryKey).not.toEqual(sessionQueries.list("student-maria", { page: 0 }).queryKey);
  });

  it("uses both identifiers for detail", async () => {
    getMock.mockResolvedValue({ data: { id: "session-1" } as never, error: undefined, response: new Response(null, { status: 200 }) });
    await getStudentSession("student-1", "session-1");
    expect(getMock).toHaveBeenCalledWith("/api/v1/teacher/students/{studentId}/sessions/{sessionId}", { params: { path: { studentId: "student-1", sessionId: "session-1" } } });
  });
});
