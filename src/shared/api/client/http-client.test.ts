...
import { apiTransport, getCsrfToken, resetCsrfToken, setUnauthorizedHandler } from "./http-client";
...
describe("apiTransport", () => {
  ...
  it.each(["POST", "PATCH", "DELETE"] as const)("gets and attaches a CSRF token for %s", async (method) => {
    const storageSpy = vi.spyOn(Storage.prototype, "setItem");
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(Response.json({ token: "csrf-token", headerName: "X-XSRF-TOKEN" }))
      .mockResolvedValueOnce(new Response(null, { status: 204 }));
    vi.stubGlobal("fetch", fetchMock);

    await apiTransport(new Request("http://localhost/api/v1/teacher/resource", { method }));
  ...
  it("refreshes the CSRF token and retries once after CSRF_INVALID", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(Response.json({ token: "stale-token", headerName: "X-XSRF-TOKEN" }))
      .mockResolvedValueOnce(Response.json({ code: "CSRF_INVALID", message: "Invalid", details: [] }, { status: 403 }))
      .mockResolvedValueOnce(Response.json({ token: "fresh-token", headerName: "X-XSRF-TOKEN" }))
      .mockResolvedValueOnce(new Response(null, { status: 204 }));
    vi.stubGlobal("fetch", fetchMock);

    await apiTransport(new Request("http://localhost/api/v1/teacher/resource", { method: "POST" }));
  ...
  it("does not retry an ordinary role-based 403", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(Response.json({ token: "csrf-token", headerName: "X-XSRF-TOKEN" }))
      .mockResolvedValueOnce(Response.json({ code: "ACCESS_DENIED", message: "Denied", details: [] }, { status: 403 }));
    vi.stubGlobal("fetch", fetchMock);

    const response = await apiTransport(new Request("http://localhost/api/v1/teacher/resource", { method: "DELETE" }));
  ...
  it("deduplicates concurrent CSRF initialization", async () => {
    const fetchMock = vi.fn().mockResolvedValue(Response.json({ token: "csrf-token", headerName: "X-XSRF-TOKEN" }));
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("notifies the app for an expired protected request but not auth discovery or login", async () => {
    ...
    const fetchMock = vi
      .fn()
      .mockResolvedValue(Response.json({ code: "AUTH_REQUIRED", message: "Required" }, { status: 401 }));
